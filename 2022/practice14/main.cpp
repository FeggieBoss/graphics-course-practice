...
int main() try
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // task1
    std::vector<GLuint> query_id;
    std::vector<bool> query_not_ready;    
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // task3
    std::vector<glm::vec3> shifts;
    for(int i=-16;i<16;++i) {
        for(int j=-16;j<16;++j) {
            shifts.push_back(glm::vec3(float(i), 0.f, float(j)));
        }
    }
    GLuint vbo_shifts;
    ...
    while (running)
    {
        ...
        for(int i=0;i<query_id.size();++i) {
            GLuint id = query_id[i];

            if(query_not_ready[i]) continue;

            cur_id = id;
            query_not_ready[i] = true;
            break;
        }

        if(cur_id==-1) {
            glGenQueries(1, &cur_id);
            query_id.push_back(cur_id);
            query_not_ready.push_back(true);
        }
        
        glBeginQuery(GL_TIME_ELAPSED, cur_id);
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        /*
        {
            ...
            //for(int i=-16;i<16;++i) {
            //    for(int j=-16;j<16;++j) {
            //        model = glm::translate(glm::mat4(1.f), {float(i), 0.f, float(j)});
            //        glUniformMatrix4fv(model_location, 1, GL_FALSE, reinterpret_cast<float *>(&model));
            //        glDrawElements(GL_TRIANGLES, mesh.indices.count, mesh.indices.type, reinterpret_cast<void *>(mesh.indices.view.offset));
            //    }
            //}
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // task4 
            //glDrawElementsInstanced(GL_TRIANGLES, mesh.indices.count, mesh.indices.type, reinterpret_cast<void *>(mesh.indices.view.offset), shifts.size());
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // task5
            frustum frustum_ = frustum(projection * view);

            std::vector<glm::vec3> shifts_to_draw;
            for(glm::vec3 &shift : shifts) {
                bool flag=true;


                aabb aabb_ = aabb(input_model.meshes[0].min+shift,input_model.meshes[0].max+shift);
                flag = intersect(aabb_, frustum_);

                if(flag)shifts_to_draw.push_back(shift);
            }
            std::cout<<"Wanna draw "<<shifts_to_draw.size()<<'\n';

            glBindBuffer(GL_ARRAY_BUFFER, vbo_shifts);
            glBufferData(GL_ARRAY_BUFFER, shifts_to_draw.size()*sizeof(glm::vec3), shifts_to_draw.data(), GL_STATIC_DRAW);
            glDrawElementsInstanced(GL_TRIANGLES, mesh.indices.count, mesh.indices.type, reinterpret_cast<void *>(mesh.indices.view.offset), shifts_to_draw.size());
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        }
        */
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // task6
        std::vector<std::vector<glm::vec3>> shifts_lod(6);
        for(glm::vec3 &shift : shifts) {
            int lod = glm::length(camera_position - ((input_model.meshes[0].min+input_model.meshes[0].max)/2.f + shift)) / 3.f;
            shifts_lod[std::min(lod,5)].push_back(shift);
        }
        ...
        for(int i=0;i<=5;++i) {
            ...
            std::vector<glm::vec3> shifts_to_draw;
            for(glm::vec3 &shift : shifts_lod[i]) {
                bool flag=true;


                aabb aabb_ = aabb(input_model.meshes[i].min+shift,input_model.meshes[i].max+shift);
                flag = intersect(aabb_, frustum_);

                if(flag)shifts_to_draw.push_back(shift);
            }

            std::cout<<"Wanna draw with lod="<<i<<" "<<shifts_to_draw.size()<<'\n';
