...
void main()
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // task3
    vec3 p = camera_position + direction_from_camera * (tmin+tmax)/2;
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // task4
    optical_depth = 0.0;
    int n = 32;
    ...
    for(int i=0;i<n;++i) {
        ...
        vec2 light_tmin_tmax = intersect_bbox(p, light_direction);
        ...
        for(int j=0;j<m;++j) {
            ...
            light_optical_depth += extinction * read_texture(l_p) * dl;
        }
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // task6
    vec3 v_light_color = vec3(30.0);
    vec3 v_scattering = vec3(8.f, 0.8, 5.f);
    vec3 v_absorption = vec3(absorption);
    vec3 v_extinction = v_absorption + v_scattering;

    n = 32, m = 8;
    dt = (tmax - tmin) / n;

    color = vec3(0.0);
    vec3 v_optical_depth = vec3(0.0);
    for(int i=0;i<n;++i) {
        float t = tmin + (i + 0.5) * dt;
        p = camera_position + t * direction_from_camera;
        v_optical_depth += v_extinction * read_texture(p) * dt;

        vec3 v_light_optical_depth = vec3(0.0);
        vec2 light_tmin_tmax = intersect_bbox(p, light_direction);
        float light_tmin = light_tmin_tmax.x;
        float light_tmax = light_tmin_tmax.y;
        light_tmin = max(light_tmin, 0.0);
        ...
        for(int j=0;j<m;++j) {
            ...
            v_light_optical_depth += v_extinction * read_texture(l_p) * dl;
        }
    ...
    out_color = vec4(vec3((tmax - tmin)/4), 1.0); // task1
    out_color = vec4(1.0, 0.0, 0.5, opacity); // task2
    out_color = vec4(read_texture(p), 0.0, 0.5, 1.0); // task3
    out_color = vec4(1.0, 0.0, 0.5, opacity); // task4
    out_color = vec4(color, opacity); // task5
    out_color = vec4(color*v_opacity, 1.0); // task6
}
)";
...
{
    int width=128, height=64, depth=64;
...
int main() try
{
    ...
    while (running)
    {
        ...
        glClearColor(0.0f, 0.0f, 0.0f, 0.f);//task6
